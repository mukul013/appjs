...
function requestListPath(
  ...
  document.getElementById(
    "update"
  ).innerHTML = `<div class='alert alert-info' role='alert'> Connecting...</div></div></div>`;
  if (fallback) {
    path = "/0:fallback";
  }
    ...
    fetch(fallback ? "/0:fallback" : path, {
      ...
      .then(function (res) {
        if (res && res.error && res.error.code === 401) {
          ...
          document.getElementById(
            "list"
          ).innerHTML = `<div class='alert alert-danger' role='alert'> Server didn't send any data.</div></div></div>`;
          $("#update").hide();
      ...
      .catch(function (error) {
        if (retries > 0) {
          ...
          document.getElementById(
            "update"
          ).innerHTML = `<div class='alert alert-info' role='alert'> Retrying...</div></div></div>`;
          performRequest(
            path,
            requestData,
            resultCallback,
          ...
          document.getElementById(
            "update"
          ).innerHTML = `<div class='alert alert-danger' role='alert'> Unable to get data from the server. Something went wrong.</div></div></div>`;
          document.getElementById("list").innerHTML =
            `<div class='alert alert-danger' role='alert'> We were unable to get data from the server. ` +
            error +
            `</div></div></div>`;
          $("#update").hide();
        }
    ...
    fetch(`/${window.current_drive_order}:search`, {
      ...
      .then(function (res) {
        if (res && res.data === null) {
          $("#spinner").remove();
          $("#list").html(
            `<div class='alert alert-danger' role='alert'> Server didn't send any data.</div></div></div>`
          );
          $("#update").remove();
        }
        if (res && res.data) {
...
function list(path, id = "", fallback = false) {
  console.log(id);
  var containerContent = `<div class="container">${
    UI.fixed_header ? "<br>" : ""
  }
    <div id="update"></div>
  ...
  if (navarray.length > 1) {
    for (var i in navarray) {
      ...
      var displayedPathPart =
        trimmedPathPart.length > 15
...
function askPassword(path) {
  ...
  var passwordInput = prompt(
    "Directory encryption, please enter the password",
    ""
  );
  ...
  try {
    ...
    for (i in files) {
      ...
      if (item["mimeType"] == "application/vnd.google-apps.folder") {
        ...
        };" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${
          item.name
        } ${
          UI.display_time
        ...
        var is_file = true;
        var epn = item.name;
        var link = UI.second_domain_for_dl
        ...
        html += `<div class="list-group-item list-group-item-action">${
          UI.allow_selecting_files
            ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' +
              link +
              '" id="flexCheckDefault">'
            : ""
        }`;
    ...
    for (i in files) {
      ...
      if (item["mimeType"] == "application/vnd.google-apps.folder") {
        html += `<a style="color: ${
          ...
          item["id"]
        }', false)" data-bs-toggle="modal" data-bs-target="#SearchModel" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${
          item.name
        } ${
          UI.display_time
            ? `<span class="badge bg-info float-end"> ` +
        ...
        html += `<div style="color: ${
          UI.css_a_tag_color
        };" gd-type="$item['mimeType']}" class="countitems size_items list-group-item list-group-item-action">${
          UI.allow_selecting_files
            ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' +
              link +
              '" id="flexCheckDefault">'
        ...
        html += ` <span onclick="onSearchResultItemClick('${
          item["id"]
        }', true)" data-bs-toggle="modal" data-bs-target="#SearchModel">${
          item.name
        }</span>${
          UI.display_download
            ? `<a href="${link}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>`
            : ``
        }<span class="badge float-end csize"> ${
          UI.display_size
            ? `<span class="badge bg-primary float-end"> ` +
              item["size"] +
              ` </span>`
...
function onSearchResultItemClick(file_id, can_preview) {
  ...
  fetch(`/${cur}:id2path`, {
    ...
    .then(function (obj) {
      ...
      title = `Result`;
      $("#SearchModelLabel").html(title);
      content = `<a class="btn btn-info" href="${encodedUrl}${
        can_preview ? "?a=view" : ""
      }">Open</a> <a class="btn btn-secondary" href="${encodedUrl}${
        can_preview ? "?a=view" : ""
    ...
    .catch(function (error) {
      ...
      $("#SearchModelLabel").html(title);
      content = `<a class="btn btn-info" href="/fallback?id=${file_id}&${
        can_preview ? "a=view" : ""
      }">Open</a> <a class="btn btn-secondary" href="/fallback?id=${file_id}&${
        can_preview ? "a=view" : ""
      }" target="_blank">Open in New Tab</a>`;
      $("#modal-body-space").html(content);
    });
      ...
      .then(function (obj) {
        ...
        if (mimeType === "application/vnd.google-apps.folder") {
          ...
          if (mimeType.includes("video") || video.includes(fileExtension)) {
            const poster = obj.thumbnailLink
              ? obj.thumbnailLink.replace("s220", "s0")
              : UI.poster;
            file_video(
              name,
              encoded_name,
...
function file_others(name, encoded_name, size, url, file_id, cookie_folder_id) {
  ...
  const copyFileBox = UI.allow_file_copy
    ? generateCopyFileBox(file_id, cookie_folder_id)
    : "";
  ...
  for (var i = 0; i < pathParts.length; i++) {
    ...
    navigation +=
      '<a href="' + new_path + '" class="breadcrumb-item">' + part + "</a>";
  }
...
function file_code(
  ...
  $("#content").html(content);
  var spinner =
    '<div class="d-flex justify-content-center"><div class="spinner-border m-5" role="status"><span class="sr-only"></span></div></div>';
  ...
  if (bytes <= 1024 * 1024 * 2) {
    ...
    $("#code_spinner").html("");
    $("#editor").html(
      `<div class="${UI.file_view_alert_class}" id="file_details" role="alert">File size is too large to preview, Max Limit is 2 MB</div>`
    );
  }
}
...
function file_video(
  ...
  if (!UI.disable_player) {
    if (player_config.player == "plyr") {
      ...
      player_js =
        "https://cdn.plyr.io/" +
        player_config.plyr_io_version +
        "/plyr.polyfilled.js";
      player_css =
        "https://cdn.plyr.io/" + player_config.plyr_io_version + "/plyr.css";
      ...
      player_js =
        "https://vjs.zencdn.net/" + player_config.videojs_version + "/video.js";
      player_css =
        "https://vjs.zencdn.net/" +
        player_config.videojs_version +
        "/video-js.css";
      ...
      <div class="card text-center">
        ...
        ${
          ...
          <div class="btn-group text-center">
              ...
              <a class="dropdown-item" href="vlc://${url}">VCL Mobile</a>
...
function file_audio(name, encoded_name, size, url, file_id, cookie_folder_id) {
    ...
    <div class="container text-center"><br>
      ...
      <div class="card text-center">
        <div class="text-center">
          <div class="${
            UI.file_view_alert_class
          }" id="file_details" role="alert">${name}<br>${size}</div>
          ${
            UI.disable_player
              ? ``
              : `
  ...
  var videoJsScript = document.createElement("script");
  videoJsScript.src =
    "https://vjs.zencdn.net/" + UI.videojs_version + "/video.min.js";
  videoJsScript.onload = function () {
    // Video.js is loaded, initialize the player
  ...
  var videoJsStylesheet = document.createElement("link");
  videoJsStylesheet.href =
    "https://vjs.zencdn.net/" + UI.videojs_version + "/video-js.css";
  videoJsStylesheet.rel = "stylesheet";
  document.head.appendChild(videoJsStylesheet);
}
...
function updateCheckboxes() {
  ...
  if (checkboxes.length > 0 && selectAllCheckbox) {
    // Check if checkboxes and selectAllCheckbox exist
  ...
  try {
    ...
    const response = await fetch("/copy", {
      ...
      body: `id=${encodeURIComponent(
        driveid
      )}&root_id=${user_folder_id}&resourcekey=null&time=${time}`,
    });
